/*
    VIEW : 가상의 테이블 (RESULTEST) SELECT 문을 이용해서 실제 테이블에서 데이터를 가져와서 사용
    VIEW ORACLE OBJECT로 DDL  구문을 이용해서 생성, 수정, 삭제
...
-- VIEW를 생성하기 위해서는 권한을 부여햐야함(RESOURCE 권한에 포함 X)     
-- 접속계정 변경(관리자)
GRANT CREATE VIEW TO KH;
-- VIEW 는 FROM절에서 테이블인것 처럼 사용 가능.
...
--- 뷰 특징
--- 1. 컬럼에 대해 산술연산 및 함수의 실행결과를 포함 할 수 있다. 단, 반드시 별칭을 부여해야함.
...
-- DECODE 함수를 이용해서 사원의 이름, 사번, 직업명, 남자인지 여자인지 성별 정보, 근속년수를 VIEW 로 저장
CREATE OR REPLACE VIEW V_EMP_JOB
AS SELECT EMP_ID
        , EMP_NAME
        , JOB_NAME
        , DECODE(SUBSTR(EMP_NO,8,1) , '1' , '남자'
                                    , '2' , '여자') AS "성별"
...
/*
    ...
    만약에 내가 업데이트 하고자 하는 칼럼이 가상칼럼이라면 DML이 불가능함.
*/
...
/*
    ...
    2. 뷰에 포함되지 않은 컬럼중 베이스가 되는 칼럼에 NOT NULL 제약조건이 설정되어 있을때
...
UPDATE V_READONLY SET EMP_NAME = '민경민';
