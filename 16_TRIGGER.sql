/*
    <트리거>
    내가 지정한 테이블에 INSERT, DELETE등의 DML문에 의해 변경사항이 발생하였을때
    자동으로 매번 실행할 내용을 미리 정의해둘수 있는 객체.
    
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터 DELETE후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리해야한다.
    신고횟수가 일정 수를 넘었을때 묵시적으로 해당 회원을 블랙리스트 처리되게끔.
    
    입출고에 대한 데이터가 기록될때마다(INSERT될때마다) 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야 될때 등.
    ...
    *트리거 종류
    ...
    SQL문에 의해 영양을 받는 각 행에 따른 분류
    > STATEMENT TRIGGER(문장트리거) : 이벤트가 발생한 SQL문에 대해 딱 한번만 트리거 실행
    > ROW TRIGGER(행 트리거) : 해당 SQL문이 실행될때마다 매번 트리거 실행(FOR EACH ROW 옵션 기술해야됨)
...
SELECT SEQ_PCODE.NEXTVAL FROM DUAL;
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.CURRVAL, '갤럭시Z플립4','삼성',99000,DEFAULT);
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '갤럭시 S23','삼성',1500000,10);
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '아이폰14','애플',1400000,DEFAULT);
SELECT * FROM TB_PRODUCT;
COMMIT;

--2. 상품 입출고 상세 이력 테이블(TB_PRODETAIL)
--   어떤 상품이 어떤날짜에 몇개가 입고, 출고가 되었는지 정보를 기록
...
-- 이력번호로 매번 새로운 번호를 발생시켜 들어갈수 있게 도와주는 시퀀스
...
/*
    ...
    TB_PRODUCT 테이블에 재고수량이 자동으로 UPDATE되게끔 트리거 만들기
    
    --상품이 입고된 경우 -> 해당 상품을 찾아서 재고수량을 증가 UPDATE
...
BEGIN 
    ...
    -- 상품이 입고된 경우
        UPDATE TB_PRODUCT
           SET STOCK = STOCK + :NEW.AMOUNT -- NEW키워드활용해서 TB_PRODETAIL에 인서트된 값을 활용할 수 있다.
...
END;
/
-- 215번 상품이 오늘날짜로 7새 출고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DCODE.NEXTVAL,210,SYSDATE,7,'출고');
SELECT * FROM TB_PRODUCT;

-- 215번 상품이 오늘날짜로 100개 입고
INSERT INTO TB_PRODETAIL
...
/*
    트리거 장점
    1. 데이터 추가, 수정, 삭제시 자동으로 데이터관리를 해줌으로써 무결성 보장
    2. 데이터베이스 관리의 자동화
    
    트리거 단점
    1. 빈번한 추가, 수정, 삭제시 ROW단위의 삽입, 추가 삭제가 함께 실행 되므로 성능상 좋지 못함.
    2. 관리적 측면에서 형상관리가 "불가능"하기 때문에 관리에 불편하다.
    3. 트리거를 남용하게 되면 예상치 못한 사태가 발생할 수 있으며 유지보수가 힘들다.
*/
