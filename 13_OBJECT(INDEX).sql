/*
    ...
    만약에 목차가 없다면, 내가원하는 챕터, 페이지가 어디있는지 책을 하나하나 훑어 봐야함.
    
    마찬가지로 테이블에서 JOIN 혹은  서브쿼리로 데이터를 조회할때
    인덱스가 없다면 테이블의 모든 데이터(FULL-SCAN)를 하나하나 뒤져서 내가 원하는 데이터를 가져올것.
...
-- 현재계정에 생성된 인덱스들 확인
SELECT * FROM USER_INDEXES; -- PK설정시 자동으로 인덱스가 생성됨

-- 현재계정에서 인덱스와 인덱스가 정용된 칼럼을 확인
...
SELECT * 
 ...
 WHERE EMP_NAME = '선동일';
-- 인덱스를 생성했다고해서 그 인덱스를 활용할지 말지는 옵티마이저가 판단을 하게됨
...
/*
    인덱스를 효율적으로 쓰기 위한 방법.
    데이터의 분포도가 높고, 조건절에 자주호출되며, 중복값이 적은 칼럼 => PK
    
    1)조건절에 자주 등장하는 컬럼
    2)항상 = 으로 비교되는 칼럼
    3)중복되는 데이터가 최소한인 칼럼(==분포도가 높음)
    4)ORDER BY 절에 자주 사용되는 칼럼
    5)JOIN조건으로 자주 사용되는 칼럼
    
    인덱스의 장점
    
    1)WHERE 절에 인덱스의 칼럼을 사용하게 되면 훤씬 빠르게 연산 할 수 있음
    
    2)ORDER BY 연산을 사용할 필요가 없다.(이미 정렬 되어 있음)
    
    3) MIN, MAX등의 집계함수로 값을 찾을 때 연산속도가 매우 빠름.(정렬되어 있기 때문에)
    
    인덱스의 단점
    1) DML에 취약함
    INSERT, UPDATE , DELETE 등 데이터가 새롭게 추가, 삭제가 되면 인덱스 테이블안에 잇는 값들도 다시 정렬하고,
    ROWID(물리적주소)값도 수정해 줘야함.
    
    2)INDEX를 이용한 INDEX-SCAN보다 단순한 FULL-SCAN이 더 유리할 때가 있다.
    일반적으로 테이블의 전체 데이터중 10~15%의 데이터를 처리해야하는 경우에만 인덱스가 효율적임
