...
--EMPLOYEE테이블로 부터 직원명과 월급, 연봉
SELECT EMP_NAME,
       SALARY,
       SALARY*12
  FROM EMPLOYEE;
  
--EMPLOYEE 테이블로 무터 직원명과 월급 보너스, 보너스가 포함된 연봉.(SALARY + SALARY*BONUS)*12
SELECT EMP_NAME,
       SALARY*BONUS,
       SALARY*(1+BONUS)*12
  FROM EMPLOYEE;
...
/*
    ...
    실제 그 테이블에 존재하는 데이터처럼 조회가 가능하다. (홀따옴표인것 주의)
*/
...
SELECT EMP_NAME,
 ...
 WHERE --DEPT_CODE != 'D9';
       --DEPT_CODE ^= 'D9';
       DEPT_CODE <> 'D9'; -- 표준 (모든 DBMS에 사용)
       
----------------실습문제--------------------

-- 1. EMPLOYEE 테이블에서 급여가 300만원이상인 사원들의 이름, 급여, 입사일 조회
SELECT EMP_NAME,
       SALARY,
       HIRE_DATE
  FROM EMPLOYEE
...
SELECT EMP_NAME,
 ...
 -- SELECT 절에서 부여한 별칭은 WHERE 절에서 사용할 수 없다. // inline 쿼리로 가능.
...
-- EMPLOYEE 테이블에서 부서코드가 D9 이면서 급여가 500만원이상인 사원들의 이름, 부서코드, 급여 조회
...
SELECT EMP_NAME,
       ...
       SALARY
  FROM EMPLOYEE
 WHERE (DEPT_CODE = 'D6' OR SALARY >= 3000000) AND ENT_YN = 'N'; -- OR조건일경우 괄호를 치는 습관들이는게 좋음
       
--급여가 350만원이상이고 600 만원 이하인 사원들의 이름, 사번, 급여, 직급코드 조회
SELECT EMP_NAME,
       EMP_ID,
       SALARY,
       JOB_CODE
...
SELECT EMP_NAME,
 ...
 --입사일이 '09/01/01' ~ '03/01/01'인 사원들의 모든 컬럼 조회 
 SELECT *
   FROM EMPLOYEE
...
/*
    ...
    - 옵션 : 특정패턴 부분에 와일드카드인 '%'. '_'를 가지고 제시할 수 있음.
       '%' : 0글자 이상
             ...
             비교대상 칼럼명 LIKE '%문자%' -> 칼럼값중에 문자가 포함되는 것을 조회
       '_' : 1글자         
             비교대상 칼럼명 LIKE '_문자' => 칼럼값 중에 '문자'앞에 무조건 1글자가 존재하는 경우 조회
             비교대상 칼럼명 LIEK '__문자' => 칼럼값중에 '문자'앞에 무조건 2글자가 존재하는 경우 조회  
  
*/
-- 내가 찾고자하는 문자가 _ 나 %일 경우
...
-- 사수가 없고 부서도 배치받지 못한 사원들의 모든 칼럼조회
SELECT *
  FROM EMPLOYEE
 WHERE MANAGER_ID IS NULL AND DEPT_CODE IS NULL;

-- 부서배치는 받지 않았지만 보너스는 받는 사원의 모든 컬럼조회
SELECT *
  FROM EMPLOYEE
...
/*
    <IN>
    비교 대상 칼럼 값에 내가 제시한 목록들중 일치하는 값이 있는지 판다.
    
    [표현법]
    칼럼명 IN (값, 값, 값, 값,...) // 동등비교를 여러번하는것
*/

--부서코드가 D6,D8,D5중 하나인 사원들의 이름, 부서코드, 급여 조회
SELECT EMP_NAME,
       DEPT_CODE,
       SALARY
  FROM EMPLOYEE
 ...
 /*
 WHERE DEPT_CODE = 'D6' OR DEPT_CODE = 'D8' OR DEPT_CODE = 'D5'; 와 같음.
 */
...
------------------------------------------------------------------------
  <ORDER BY 절>
  SELECT 문 가장 마지막에 기입하는 구문 뿐 만 아니라 가장 마지막에 실행되는 구문
  ...
  SELECT 컬럼명1,
    ...
    FROM 테이블명
   WHERE 조건식(생략가능)
   ORDER BY [정렬기준으로 세우고자 하는 칼럼명/별칭/컬럼순번] [ASC/DESC](생략시 ASC오름차순) [NULLS FIRST/NULLS LAST](생략가능)
*/

--월급이 높은 사람들 부터 나열하고 싶다.
SELECT *
  FROM EMPLOYEE
...
SELECT *
  FROM EMPLOYEE
 --ORDER BY BONUS; -- ASC, NULLS LAST : 기본값
 --ORDER BY BONUS ASC NULLS FIRST;
 --ORDER BY BONUS DESC; -- NULLS FIRST : 기본값.
 ORDER BY BONUS DESC, SALARY ASC;
 --> 첫번째로 제시한 정렬기준 컬럼값이 일치할 경우 두번째 정렬기준을 가지고 다시 정렬. // 게시판 정렬기준, 리소스많이 먹음 (FROM문 1차 풀스캔 -> ORDER BY 2차 풀스캔하면서 첫번째 조건으로 정렬 -> , 2번째 조건 만나면서 3차 풀스캔하면서 2번째 조건 정렬
 --> SELECT했을때 자동으로 정렬이 되는 방법을 씀.
